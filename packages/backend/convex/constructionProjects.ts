        ...
        const populatedProjects = await Promise.all(
            projects.map(async (project) => {
                const [status, lead, priority, monthlyRevenue] = await Promise.all([
                    ...
                    ctx.db.query("monthlyRevenue")
                        .withIndex("by_project", (q) => q.eq("constructionProjectId", project._id))
                        .collect(),
                ]);
...
export const addMonthlyRevenue = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_project", (q) => q.eq("constructionProjectId", args.constructionProjectId))
