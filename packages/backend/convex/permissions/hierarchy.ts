...
export async function checkHierarchicalPermission(
  ...
  const hasDirectPermission = await checkPermission(ctx, userId, resource, action);
  ...
  if (userPosition.level < targetPosition.level && userPosition.canManageLevelsBelow) {
...
export async function getManagedUsers(
  ...
  const managedDepartments = await getSubDepartments(ctx, userDepartment.departmentId);
