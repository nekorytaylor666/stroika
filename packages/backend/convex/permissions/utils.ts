...
export async function checkPermission(
  ...
  for (const rolePerm of rolePermissions) {
    const permission = await ctx.db.get(rolePerm.permissionId);
    if (permission && permission.resource === resource && permission.action === "manage") {
      return true;
    }
  }
...
export async function getUserPermissions(
  ...
  const permissions: Array<{ resource: PermissionResource; action: PermissionAction }> = [];
...
export async function requirePermission(
  ...
  if (!hasPermission) {
    throw new Error(`Unauthorized: Missing permission for ${action} on ${resource}`);
  }
}
