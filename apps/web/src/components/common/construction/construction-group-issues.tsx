...
const ConstructionIssueGridList: FC<{ issues: Issue[]; status: Status }> = ({ issues, status }) => {
    const ref = useRef<HTMLDivElement>(null);
    const { updateConstructionIssueStatus } = useConstructionStore();
    ...
    const [{ isOver }, drop] = useDrop(() => ({
        ...
        drop(item: Issue, monitor) {
            if (monitor.didDrop() && item.status.id !== status.id) {
                updateConstructionIssueStatus(item.id, status);
            }
        },
        collect: (monitor) => ({
