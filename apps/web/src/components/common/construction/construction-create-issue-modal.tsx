...
export function ConstructionCreateIssueModal() {
    const { isOpen, defaultStatus, closeModal } = useConstructionCreateIssueStore();
    ...
    useEffect(() => {
        if (priorities?.length > 0 && !priority) {
            setPriority(priorities.find(p => p.name === 'Средний') || priorities[0]);
        }
        if (statuses?.length > 0 && !issueStatus) {
            setIssueStatus(defaultStatus || statuses.find(s => s.name === 'К выполнению') || statuses[0]);
        }
