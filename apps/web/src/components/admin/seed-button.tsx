  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      toast.success("База данных успешно инициализирована", {
        description: "Создано ролей, разрешений, отделов, пользователей, проектов и задач",
      });
      
      // Show detailed results
      if (result.results && Array.isArray(result.results)) {
        result.results.forEach((step: { step: string; message?: string; error?: string }) => {
          if (step.error) {
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            <p className="mt-3 font-medium">
              Внимание: Это действие не удаляет существующие данные, но может создать дубликаты при повторном запуске.
            </p>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
