...
// Utility functions
export function groupProjectsByStatus(projects: ConstructionProject[]): Record<string, ConstructionProject[]> {
...
export function sortProjectsByPriority(projects: ConstructionProject[]): ConstructionProject[] {
    const priorityOrder = { urgent: 0, high: 1, medium: 2, low: 3, 'no-priority': 4 };
    return [...projects].sort((a, b) => priorityOrder[a.priority.id] - priorityOrder[b.priority.id]);
}

export function calculateTotalContractValue(projects: ConstructionProject[]): number {
    return projects.reduce((total, project) => total + project.contractValue, 0);
}

export function getMonthlyRevenueTotal(projects: ConstructionProject[], month: string): { planned: number; actual: number } {
