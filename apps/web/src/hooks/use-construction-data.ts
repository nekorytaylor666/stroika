...
export const useConstructionData = () => {
    ...
    const updateProjectStatus = useMutation(api.constructionProjects.updateStatus);
    const updateProjectProgress = useMutation(api.constructionProjects.updateProgress);
    ...
    // Loading state
    const isLoading = projects === undefined || teams === undefined || tasks === undefined ||
        users === undefined || labels === undefined || priorities === undefined ||
        statuses === undefined;
    ...
    const filterTasks = (filters: any) => {
        ...
        if (filters.priority && filters.priority.length > 0) {
            result = result.filter(task => filters.priority.includes(task.priorityId));
        }
    ...
    const handleMoveTask = async (id: Id<"issues">, newStatusId: Id<"status">, newRank: string) => {
        try {
