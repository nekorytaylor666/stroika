import { groupProjectsByStatus, sortProjectsByPriority, type ConstructionProject, constructionProjects as mockProjects, calculateTotalContractValue, getMonthlyRevenueTotal, constructionProjects } from '@/mock-data/construction/construction-projects';
import { type ConstructionTeam, constructionTeams } from '@/mock-data/construction/construction-teams';
import { constructionTasks, groupConstructionTasksByStatus } from '@/mock-data/construction/construction-tasks';
...
interface ConstructionState {
    ...
    updateProject: (id: string, updatedProject: Partial<ConstructionProject>) => void;
    ...
    updateConstructionIssuePriority: (issueId: string, newPriority: Priority) => void;
    updateConstructionIssueAssignee: (issueId: string, newAssignee: User | null) => void;
    ...
    addMonthlyRevenue: (projectId: string, month: string, planned: number, actual: number) => void;
...
const groupConstructionIssuesByStatus = (issues: Issue[]): Record<string, Issue[]> => {
...
export const useConstructionStore = create<ConstructionState>((set, get) => ({
    ...
    deleteConstructionIssue: (id) =>
        set((state) => {
            const newIssues = state.constructionIssues.filter((issue) => issue.id !== id);
            return {
                constructionIssues: newIssues,
                constructionIssuesByStatus: groupConstructionIssuesByStatus(newIssues),
            };
    ...
    filterByStatus: (statusId) => get().projects.filter((project) => project.status.id === statusId),

    filterByPriority: (priorityId) => get().projects.filter((project) => project.priority.id === priorityId),
    ...
    filterByClient: (client) => get().projects.filter((project) =>
        project.client.toLowerCase().includes(client.toLowerCase())
    ),

    filterByProjectType: (type) => get().projects.filter((project) => project.projectType === type),

    filterByLocation: (location) => get().projects.filter((project) =>
    ...
    filterProjects: (filters) => {
        ...
        if (filters.client && filters.client.length > 0) {
            filteredProjects = filteredProjects.filter((project) =>
                filters.client!.some(client => project.client.toLowerCase().includes(client.toLowerCase()))
            );
        }
        ...
        if (filters.location && filters.location.length > 0) {
            filteredProjects = filteredProjects.filter((project) =>
                filters.location!.some(location => project.location.toLowerCase().includes(location.toLowerCase()))
            );
        }
    ...
    addMonthlyRevenue: (projectId, month, planned, actual) => {
        ...
        if (project) {
            const existingRevenue = project.monthlyRevenue.filter(r => r.month !== month);
