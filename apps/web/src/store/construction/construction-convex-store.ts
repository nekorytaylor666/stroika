...
interface ConstructionConvexState {
    ...
    moveTask: (taskId: Id<"issues">, newStatusId: Id<"status">, newRank: string) => void;

    // Project management
    addProject: (project: ConstructionProject) => void;
    updateProject: (id: Id<"constructionProjects">, updates: Partial<ConstructionProject>) => void;
    deleteProject: (id: Id<"constructionProjects">) => void;

    // Team management
    addTeam: (team: ConstructionTeam) => void;
    updateTeam: (id: Id<"constructionTeams">, updates: Partial<ConstructionTeam>) => void;
    ...
    getProjectById: (id: Id<"constructionProjects">) => ConstructionProject | undefined;
...
// Helper functions
const groupTasksByStatus = (tasks: ConstructionTask[], statuses: any[]): Record<string, ConstructionTask[]> => {
...
const groupProjectsByStatus = (projects: ConstructionProject[], statuses: any[]): Record<string, ConstructionProject[]> => {
    const grouped: Record<string, ConstructionProject[]> = {};

    statuses.forEach(status => {
        grouped[status._id] = projects.filter(project => project.statusId === status._id);
    });
        ...
        (set, get) => ({
            ...
            setProjects: (projects) => {
                ...
                set({
                    ...
                    totalContractValue: projects.reduce((total, project) => total + project.contractValue, 0),
                });
            },
            ...
            addProject: (project) => {
                ...
                set({
                    ...
                    totalContractValue: newProjects.reduce((total, p) => total + p.contractValue, 0),
                });
            },
            ...
            getPriorityById: (id) => get().priorities.find(priority => priority._id === id),
            ...
            refreshData: () => {
                ...
                set({
                    projectsByStatus: groupProjectsByStatus(projects, statuses),
                    tasksByStatus: groupTasksByStatus(tasks, statuses),
                    totalContractValue: projects.reduce((total, p) => total + p.contractValue, 0),
                });
            },
        }),
        {
